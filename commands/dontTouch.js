...
module.exports.run = async (bot, message, args) => {
  ...
  if(!args[0])return message.channel.send("Veuillez préciser un utilisateur.").then(msg => {
    msg.delete({ timeout: 10000 })
  })
  .catch(/*Your Error handling if the Message isn't returned, sent, etc.*/);
  let toSave = message.guild.member(message.mentions.users.first() || message.guild.members.gets(args[0]));
  if(!toSave) return message.channel.send("L'utilisateur n'a pas été trouvé.").then(msg => {
    msg.delete({ timeout: 10000 })
  })
  .catch(/*Your Error handling if the Message isn't returned, sent, etc.*/);
  ...
  message.channel.send(`${toSave} est maintenant protégé.`).then(msg => {
    msg.delete({ timeout: 10000 })
  })
  .catch(/*Your Error handling if the Message isn't returned, sent, etc.*/);
  return;
