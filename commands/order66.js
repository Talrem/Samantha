...
if(message.guild.id!="311112661108785153") return message.reply("Cette commande n'est pas disponible sur ce serveur");
  ...
  if(!message.member.permissions.has("ADMINISTRATOR")) return message.channel.send("Vous n'avez pas le droit de faire ça.");
  let i;
  // Get the Guild and store it under the variable "list"
  const list = bot.guilds.get("311112661108785153");
  // Iterate through the collection of GuildMembers from the Guild getting the username property of each member
  list.members.forEach(member => {
    if(member.roles.has("512016395089870884") && !member.permissions.has("ADMINISTRATOR")){
